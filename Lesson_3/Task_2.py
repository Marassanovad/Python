...
def find_second_encounter(word, words_list):
    if words_list.count(word) > 1:
        ...
        print(f"второе вхождение: {words_list.index(word, first_encounter + 1)+1}")
    else:
        print("-1")


count, source = int(input('Введите число: ')), input('Введите буквы: ')

words = form_words_list(count, source)
print(words)
find_second_encounter(input('Введите слово: '), words)
